...
case "$TRAVIS_OS_NAME" in
  linux)
    ...
    if [[ "x$TARGET" = "xmingw-w64" ]]; then
      CMAKE_OPTIONS="-DOSSIA_PD_PATH=${TRAVIS_BUILD_DIR}/pd -DCROSS_COMPILER_PATH=${TRAVIS_BUILD_DIR}/mingw-w64-install/ -DCMAKE_TOOLCHAIN_FILE=${TRAVIS_BUILD_DIR}/CMake/toolchains/mingw-w64.cmake"
